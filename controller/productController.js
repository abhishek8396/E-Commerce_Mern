...
export const getProductController=async(req,res)=>{
    try {
        const product = await productModel.find({}).populate('category').select("-photo").limit(20).sort({createdAt:-1})
        res.status(200).send({
            success:true,
...
//Product Count Controller OR PAGINATION
...
//List Product Controller for Pagination
...
export const listProductController= async(req,res)=>{
    try {
        ...
        res.status(400).send({
            success:false,
            message:"Error in product Page list",
            error
        })
    }
}
